...
3. Framework preset: **Vite** (auto-detected). No env vars required.
4. Deploy. Vercel serves over HTTPS automatically — camera will work on mobile.

For other hosts (Netlify, Cloudflare Pages, etc.) the build command is `npm run build` and the output directory is `dist/` (or framework default).

## Privacy

All data stays on the device. No backend, no analytics, no network calls after initial page load.
